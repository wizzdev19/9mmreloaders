...
(function(){
  ...
  function renderClientCart(){
    ...
    var cryptoDiscount = Math.round(subtotal * 0.15);
    var cryptoTotal = subtotal - cryptoDiscount;
    html+='</tbody></table><p><button class="btn btn-secondary btn-sm" id="clear-cart">Clear cart</button></p></div><div><div class="cart-summary"><h2>Summary</h2><dl><dt>Items</dt><dd>'+itemCount+'</dd><dt>Subtotal</dt><dd>$'+(subtotal/100).toFixed(2)+'</dd><dt style="color:#0a7d0a">Crypto price (15% off)</dt><dd style="color:#0a7d0a">$'+(cryptoTotal/100).toFixed(2)+'</dd><dt>Shipping</dt><dd>Calculated at checkout to your dropoff location</dd><dt>Tax</dt><dd>Calculated at checkout</dd></dl><p class="meta-updated" style="background:#f0f8f0;border:1px solid #c3e6c3;padding:8px;border-radius:4px"><strong style="color:#0a7d0a">Save 15% with Crypto - $'+(cryptoDiscount/100).toFixed(2)+' off</strong><br>Choose Crypto at checkout and pay only $'+(cryptoTotal/100).toFixed(2)+' instead of $'+(subtotal/100).toFixed(2)+'</p><p class="meta-updated">Firearms ship to your dropoff location where the licensed dealer completes the transfer. Parts can ship directly to you.</p>';
