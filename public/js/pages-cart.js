...
(function(){
  ...
  // Always use client-side cart on static deployments
  ...
  // Intercept add to cart forms - make functional everywhere
  document.addEventListener('submit', function(e){
    var form = e.target;
    if(!form || form.getAttribute('action') !== '/cart/add') return;
    
    // On localhost with backend, try backend first but fallback to client cart
    var isLocalhost = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
    if(isLocalhost && !useClientCart) {
      // Let server handle, but add fallback after 2s if fails
      return;
    }

    // For static sites (Pages, Workers) or when forced, handle client side
    e.preventDefault();
    ...
    var priceCents = 0;
    if(priceText) {
      priceCents = Math.round(parseFloat(priceText[1].replace(/,/g,''))*100) || 0;
    }
    var img = document.querySelector('#gallery-image')?.getAttribute('src') || document.querySelector('.card-media img')?.getAttribute('src') || '';
  ...
  function renderClientCart(){
    ...
    // Check if server already rendered cart with items
    ...
    // If server has items and we're not on static, don't override
    if(hasServerItems && !useClientCart) return;
    
    // If empty message or no table, render client cart
    if(cart.length === 0) {
    ...
    // Build cart table from localStorage
    ...
    html+='</tbody></table><p><button class="btn btn-secondary btn-sm" id="clear-cart">Clear cart</button></p></div><div><div class="cart-summary"><h2>Summary</h2><dl><dt>Items</dt><dd>'+itemCount+'</dd><dt>Subtotal</dt><dd>$'+(subtotal/100).toFixed(2)+'</dd><dt>Shipping</dt><dd>Calculated at checkout to your dropoff location</dd></dl><p class="meta-updated">Firearms ship to your dropoff location where the licensed dealer completes the transfer. Parts can ship directly to you.</p><p><a class="btn btn-primary btn-block" href="/contact">Contact about these items</a></p><p><a class="btn btn-secondary btn-block" href="/glock-pistols-for-sale/">Keep browsing</a></p><p class="meta-updated"><small>Cart saved in browser. Checkout requires contacting shop.</small></p></div></div></div>';
    wrap.innerHTML = html;
  ...
  // Also handle Add to Cart buttons that are not in forms (product cards)
